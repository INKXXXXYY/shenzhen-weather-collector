...
def fetch_qweather(api_key, location, retries=3, timeout=12):
    """和风 Now API：用 obsTime 作为观测时间（精确时刻），location 支持“经度,纬度”或 ID。"""
    if not api_key:
        return None
    url = (
    ...
    for i in range(retries):
        try:
            ...
            row = {
                "ts_iso": now_cn_iso(),                    # 采集时间（本地记录）
                "ts_obs_iso": now.get("obsTime", ""),      # 观测时间（官方给的准确时间）
                "location_name": LOCATION_NAME,
                "provider": "qweather",
                ...
                "weather_desc": now.get("text"),           # 中文现象描述
            }
            return row
        except Exception:
            if i == retries - 1:
